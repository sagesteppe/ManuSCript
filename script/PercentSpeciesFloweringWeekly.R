...
bees <- read.csv('../data/Bombus_queen_observations_2015.csv') %>% 
  ...
  tally() %>% 
  filter(! species %in% c('centralis', 'melanopygus', 'kirbiellus', 'unknown'))
...
weeks <- data.frame(
  ...
  doy = 
    c(130:136, 137:143, 144:150, 151:157, 158:164, 165:171, 172:178, 179:185, 186:192, 
      193:199, 200:206, 207:213, 214:220, 221:227, 228:234, 235:241, 242:248, 249:255, 256:262)
    )
...
wkkly <- test %>% 
  ...
  tally()
wkkly$prop <- wkkly$n / nrow(distinct(test, taxon))

wkklyLow <- test %>% 
  group_by(week) %>% 
  filter(estimate == 'low_ci') %>% 
  tally()
wkklyLow$prop <- wkklyLow$n / nrow(distinct(test, taxon))

wkklyHigh <- test %>% 
  group_by(week) %>% 
  filter(estimate == 'high_ci') %>% 
  tally()
wkklyHigh$prop <- wkklyHigh$n / nrow(distinct(test, taxon))

barCenters <- barplot(wkkly$prop, horiz = F, names.arg = 3:11, ylim = c(0,1), 
        yaxt = "n", xlab = 'Week', ylab = 'Proportion', 
        main = 'Proportion of the 379 phenology\nmodelled species flowering by week')
axis(side = 2, las = 2)

segments(x0 = barCenters, y0 = wkklyLow$prop, 
         x1 = barCenters, y1= wkklyHigh$prop)
arrows(x0 = barCenters, y0 = wkklyLow$prop, 
       x1 = barCenters, y1= wkklyHigh$prop, lwd = 1.5, angle = 90,
       code = 3, length = 0.02)
