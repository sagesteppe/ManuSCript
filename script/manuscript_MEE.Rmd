...
corb <- read.csv('../data/Corbiculae_collections.csv', header = F) %>% 
  mutate(V1 = str_remove(V1, 'dark|orange')) %>% 
  filter(!str_detect(V1, 'bifarius')) %>% 
  separate(V1, into = c('sample.id', 'genus', 'species', 'caste',
                        'plant.genus', 'plant.species')) %>% 
  filter(plant.genus == 'Mertensia')
...
# # SOMETHING MUST BE VERY WRONG HERE, MISSED MERTENSIA IN NEARLY PURE SAMPLES. 
blst_mertensia <- blst %>% 
...
M.df <- data.frame(Prop = seq(0, 100, 0.5))
M.df$DETECTED <- predict(M1, newdata=M.df, type="response")

ggplot(M.df, aes(x=Prop, y=DETECTED)) +
