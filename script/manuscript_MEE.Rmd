...
summary(model <-  lm(sequenceProp ~ countProp, data = pol_p) ) # better with additive term + morphotype
summary(model)$adj.r.squared
summary(model)$p.val

newdat <- data.frame(
  countProp = seq(from = min(pol_p$countProp), to = max(pol_p$countProp), 
                  length.out = 100) )#, 
#  morphotype = sample(unique(pol_p$morphotype), replace = T, size = 100,
#       prob = as.numeric(table(pol_p$morphotype)) / nrow(pol_p)))

CI <- predict(model, newdat, se.fit = TRUE, interval = "confidence", level = 0.95)

par(pty="s")
plot(x = pol_p$countProp,  pol_p$sequenceProp, 
...
axis(2)
abline(model)
lines(newdat$countProp, CI[['fit']][,3], lty = 'dotted', col = 'grey25')
lines(newdat$countProp, CI[['fit']][,2], lty = 'dotted', col = 'grey25')
text(0.2, 0.8, expression("adj. r"^2*" = " ))
text(0.4, 0.8, paste0(round(summary(model)$adj.r.squared, 3)))
text(0.3, 0.7, "p < 0.001")

rm(newdat)
```


```{r Run with correction for total sequences, eval = F}
## doesnt really make a difference - increase r2 by about 2% - with morphotype up to 6%, multiple r2 0.8

summary(model <-  lm(adjSeqProp ~ countProp, data = pol_p) )  #  + morphotype
newdat <- data.frame(
  countProp = seq(from = min(pol_p$countProp), to = max(pol_p$countProp), 
                  length.out = 100) )
CI <- predict(model, newdat, se.fit = TRUE, interval = "confidence", level = 0.95)
...
The relationship shows that grains which are often present in trace amounts have relatively high amounts of their abundances estimated, and grains which are present in high amounts have fewer sequences classified. This is considered to be due to the proportion of high false positives which occur in the classification process with NGS (BELL NOVEMEBER 2021)
...
We have demonstrated how Angiosperms533 hyb-seq probes may be used for plant barcoding in a metagenomic context. This was exemplified in an ecologically relevant scenario, where the results have immediate implications for natural history driven fundamental science and the applied science of land management. The test pollen loads contained a number of closely related taxa, some in notoriously taxonomically difficult clades (e.g. *Mertensia*, *Lupinus*), at naturally occurring proportions.  We incorporated spatial and temporal approaches for creating custom sequence databases an approach which is readily applicable to any lab group with the capacity to perform next-generation sequencing across the entirety of multiple continents, and which we expect to be highly beneficial in many study areas. By combining insights from these novel approaches with an extensive observational field based study we show how these methods may be applied to test a variety of hypotheses related to ecological interactions. 
