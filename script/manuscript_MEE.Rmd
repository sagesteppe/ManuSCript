...
XX corbiculae loads had DNA extracted and underwent hyb-seq, a total of `r toString(toseq)` corbiculae samples were sequenced, XX reads were recovered from sequencing. After trimming XX sequences remained. `r toString(sum(krak[krak$Classified == 'C',]$n))` reads were matched using Kraken, of the reads classified by Kraken `r toString(sum(brack$reads))` reads were matched using Bracken, of the reads classified by Kraken `r toString(sum(blast$Total_seqs))` reads were matched using BLAST. The number of reads per sequence varied widely (min = , $\bar{x}$, max = ), the number of loci recovered per sample ranged from (min = , max = ), and the number of reads from loci also ranged widely () **APPENDIX X**. 
