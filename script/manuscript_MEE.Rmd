...
prcnt_detect <- function(x, Lprop, Hprop){
  ...
  return(writeLines(paste0('were detected in ',  detected, 
...
*< UTILITY OF MASS IDENTIFICATION OF MATERIAL - FRIENDLY STAGE SET >*

*< ISSUES AND COSTS WITH IDENTIFYING MATERIAL TO SPECIES >* many spp defined by interactions (<;

The inability to reliably identify plants down to species can limit our understanding of ecosystem function and interactions. This is especially true for genera where the species are well defined based upon ecological and behavioral rather than morphological properties, and hence can serve as key habitat bioindicators(e.g. different species of Sagebrush- *Artemisia* L., Willows - *Salix* L., and Sedges - *Carex* L.) (@Gage2013HistoricalRO). The lack of species level data can hinder our understanding of the breadth of habitat which some species occupy, and the interactions they have with other species (CITE). Current methods to ameliorate this situation include: ignoring these ecologically relevant levels of detail, revisiting plots as diagnostic material becomes temporally available, assistance from taxonomic specialists, or the use of barcoding or other molecular techniques.
...
In order to minimise the number of species for which SDM’s were to be generated, BIEN was queried at a distance of up to 100km from our study area and all plant species records were downloaded. **_In order to emulate the stochasticity of botanical collecting, this data set was bootstrap re-sampled 250 times, with 90% of samples selected, to create a testing data set_**. The median of the logistic regression assessing the probability of occurrence of a species record as a function of distance from the study area was used as a threshold distance, under which, to include species as candidates for distribution modelling.
...
# set up as evaluation table !?

mertensia_detected <- mertensia_samples %>% 
  mutate(DETECTED = if_else(sample.id %in% blst_mertensia$sample.id, 1, 0)) 
 # sample 26 - very strange !!!! this needs investigation

M1 <- glm(DETECTED ~ Prop, mertensia_detected, family = binomial)
M.df <- data.frame(Prop = seq(0, 1, 0.0001))
M.df$DETECTED <- predict(M1, newdata=M.df, type="response")

ggplot(mertensia_detected, aes(x=Prop, y=DETECTED)) +
  stat_smooth(method="glm", se=T, fullrange=TRUE, 
              method.args = list(family=binomial))  +
  theme_bw()

# Repeat for Asteraceae 1 & 2

blst_aster <- blst %>% 
  filter(Family == 'Asteraceae') %>% 
  select(sample.id,  Prcnt_seqs, Genus) %>% 
  group_by(sample.id) %>% 
  distinct(sample.id, .keep_all = T)
...
M1 <- glm(DETECTED ~ Prop, aster2_detected, family = binomial)
M.df <- data.frame(Prop = seq(0, 1, 0.0001))
M.df$DETECTED <- predict(M1, newdata=M.df, type="response")

ggplot(aster2_detected, aes(x=Prop, y=DETECTED)) +
  stat_smooth(method="glm", se=T, fullrange=TRUE, method.args = list(family='binomial'))  +
  theme_bw()
...
ggplot(aster1_detected, aes(x=Prop, y=DETECTED)) +
  stat_smooth(method="glm", se=T, fullrange=TRUE, method.args = list(family='binomial'))  +
  theme_bw()
...
rm(M1, M.df, M.df,  mertensia_samples, blst, blst_aster, 
   blst_aster1, blst_aster2, aster1_samples,  aster2_samples, 
   a, b, c, d, blast_mertensia, aster_samples)
```


The pollen samples contained three morphotypes which could readily be identified. Two of these mapped to monophyletic clades (Boraginaceae & Heliantheae Alliance), and one to a paraphyletic clade (Asteraceae less Heliantheae).
Boraginaceae grains `r prcnt_detect(mertensia_detected, Lprop = 0.01, Hprop = 1)`.  Asteraceae type 1, non-helianthoids, `r prcnt_detect(aster1_detected, Lprop = 0.001, Hprop = 0.01)` Asteraceae type 2, Helianthoids, `r prcnt_detect(aster2_detected, Lprop = 0.001, Hprop = 0.01)`. Both morphotypes of Asteraceae pollen `r prcnt_detect(aster_detected, Lprop = 0.01, Hprop = 1)`. 
...
Based upon subjective review of the three classifiers **APPENDIX X MOLECULAR NETWORKS - 3 DIFFERENT ONES**, BLAST was chosen as the classification method which yielded the most probable results, and it's values were used for all subsequent analyses. 
...
gf_lkp <- blst %>% select(Genus, Family) %>% distinct(Genus, .keep_all = T) %>% 
  bind_rows(gf_lkp2)
...
**overall stuff and things**
We have demonstrated how Angiosperms533 data may be used for plant barcoding in a metagenomic context. This was exemplified in an ecologically realistic situation, centered both in a context which serves as an immediate framework for land management studies, and which produced fundamental natural history information. These data sets included closely related taxa in notoriously taxonomically difficult clades (e.g. *Mertensia*, *Lupinus*). We utilized spatial and temporal approaches for creating custom sequence databases which are readily applicable to any lab group with the capacity to perform next-generation sequencing across the entirety of multiple continents. In conjunction with well designed and implemented field based data collection we show how these methods may be widely used to test a variety of hypotheses related to ecological interactions. 

These results show that the overall results between **Bumble Bee ecology** observational and barcoding are largely congruent. But that ...
Future analyses of the long term data set... Global studies should aim to corroborate ...

**Spatial ... & Timing ** (filters!)
- Fewer modelling runs for SDM's likely to be effective for determining inclusion, elastic inclusion criteria per application.
- The actual data set which was used for training and testing all of the models incorporated into SDM's represented only roughly one quarter of the records available for such purposes. We consciously chose to do this in order to showcase the possibility of this approach working in less data rich areas. 
- Used fine scale flora to evaluate results of modelling, similar approaches based on quick field surveys which do not require comprehensive floristic endeavors.
- Timing in other areas may suffer complications due to lacking as strong a signal as that induced by snow melt, but longer duration of flowering likely to help. 
- Sample size in phenology estimates had an effect which is likely to be maintained across all results.
- Regardless both show good agreement on flower onset, peak flowering, and moderate agreement with flowering cessation. The disagreement in flowering cessation is perhaps due to more microclimates which retain water, rather than microclimates which allow the early accumulation of heat. 
- Number of botanists rapidly decreasing no hope for much more field based information or funding.   

**Molecular ... **
- PAFTOL project provide complete genus level reference data set for the kingdom
- Expected to work nearly immediately for DNA barcoding of whole plant material.
- With work expected to work well in meta contexts. 
- Harvest both variable loci, and variable flanking regions of loci for fine taxonomic resolution
- As genes are nuclear, single copy, and a variety are present the possibility of identifying target loci for quantitative purposes is high.
- Potential for infra-specific inference (@wenzell2021incomplete, Loke et al.,  @slimp2021potential, @beck2021palmer). 

**Final P**
 < bayesian framework > & < harvesting loci >  < Possible Application at landscape scale >  < expert natural history >
