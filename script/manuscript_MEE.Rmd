...
author:  |
    ...
    |  $^2$Plant Biology and Conservation, Northwestern University, Evanston, IL 60208, USA,
    |  $^3$ Rocky Mountain Biological Laboratory, P.O. Box 519, Crested Butte, CO 81224, USA
...
```{r compare microscopic to molecular Pollen ID, echo = F}
...
blst_aster1 <- filter(blst_aster, !Genus %in% c('Taraxacum', 'Helianthella', 'Scabrethia') )
...
rm(mertensia_samples, blst, blst_aster, blst_aster1,
   blst_aster2, aster1_samples,  aster2_samples, a, b, c, d, 
...
```{r Summary statistics of sequence numbers, echo = F}
...
```{r clean sequence numbers from environment, eval = F, echo = F}
rm(toseq, krak, brack, blast, loci, no_loci, aa, bb, cc, dd)
```
...
```{r Floral Observations Summary Statistics, echo = F, warning = F}
...
```{r Most used resources by species, echo =F, fig.cap = 'Number of the ten most commonly visited plants which are also in the top ten most common sequences', out.width = "45%"}
...
srm(bee_cols, correct, family, genus, tblist, oblist, gf_lkp, gf_lkp2)
```
