...
# all samples with duplicate genera in them # be careful these are not easy mappings
duplicate_reps_in_sample <- blst_expert %>% 
  ...
  # now split the reads assigned to them into each constituent taxon ~ 'unwise King Soloman'
  ungroup() %>% 
  group_by(RecordID) %>% 
  mutate(Prcnt_seqs_Reclass = Prcnt_seqs/n())
  


blst_reclass_expert <- blst_expert %>% 
  filter(!RecordID %in% duplicate_reps_in_sample$RecordID) %>% 
  rowwise() %>% 
  mutate(Genus = str_trim(Genus))  %>% 

  mutate(TAXON_EXPERT = case_when(
    sample.id == 3 & Genus == 'Delphinium' ~ plant.species,
    sample.id == 4 ~ plant.species, # 4 has a viola split
    ...
    sample.id == 9 ~ plant.species, # has both Delphinium
    sample.id == 10 ~ plant.species,
    
    sample.id == 14 ~ plant.species,
    
    sample.id == 18 ~ plant.species,
    sample.id == 19 ~ plant.species,
    sample.id == 20 ~ plant.species,
    sample.id == 21 ~ plant.species,
    sample.id == 23 ~ plant.species, # has both Viola
    sample.id == 23 & Genus %in% ranunc_splash ~ 'Delphinium.nuttallianum',
    sample.id == 24 ~ plant.species,
    sample.id == 24 & Genus %in% ranunc_splash ~ 'Delphinium.nuttallianum',
    sample.id == 25 ~ plant.species,
    sample.id == 25 & Genus %in% ranunc_splash ~ 'Delphinium.nuttallianum',
    sample.id == 26 ~ plant.species,
    sample.id == 26 & Genus %in% ranunc_splash ~ 'Delphinium.nuttallianum',
  #  sample.id == 27 ~ plant.species, # has both Hydrophyllum
    sample.id == 27 & Genus %in% ranunc_splash ~ 'Delphinium.nuttallianum',
    sample.id == 28 ~ plant.species,
    sample.id == 28 & Genus %in% ranunc_splash ~ 'Delphinium.nuttallianum',
    sample.id == 29 ~ plant.species,
    sample.id == 30 ~ plant.species,
    sample.id == 30 & Genus == 'Cynoglossum' ~ 'Mertensia.fusiformis', 
    sample.id == 32 ~ plant.species,
    sample.id == 32 & TAXON_NEW %in% ranunc_splash ~ 'Delphinium.nuttallianum',
    sample.id == 33 ~ plant.species, # has both Viola
    
    sample.id == 35 ~ plant.species, # has both Viola
    sample.id == 35 & TAXON_NEW %in% ranunc_splash ~ 'Delphinium.nuttallianum',
    
    sample.id == 36 ~ plant.species,
    
    sample.id == 39 ~ plant.species, # has both hydrophyllum
    sample.id == 39 & TAXON_NEW %in% ranunc_splash ~ 'Delphinium.nuttallianum',
    
    
    sample.id == 41 ~ plant.species,
    sample.id == 41 & Genus == 'Cynoglossum' ~ 'Mertensia.ciliata', 
    
    sample.id == 42 ~ plant.species,
    sample.id == 43 ~ plant.species,
    #sample.id == 44 ~ plant.species, # 44 has both delphinium 
    
    sample.id == 46 ~ plant.species,
    sample.id == 49 ~ plant.species,
    sample.id == 49 & TAXON_NEW %in% ranunc_splash ~ 'Delphinium.barbeyi',
  #  sample.id == 51 ~ plant.species, # 51 has a viola split
    sample.id == 51 & TAXON_NEW %in% ranunc_splash ~ 'Delphinium.nuttallianum',
  
    sample.id == 52 ~ plant.species,
    sample.id == 54 ~ plant.species,
    sample.id == 55 ~ plant.species,

    sample.id == 56 ~ plant.species,
  ...
  group_by(sample.id, plant.species) %>% 
  ...
  mutate(TAXON_EXPERTS = case_when(
      TAXON_NEW %in% c('Cynoglossum amplifolium', 'Cynoglossum anchusoides', 
      'Cynoglossum officinale', 'Cynoglossum pringlei') ~ 'Mertensia',
      TAXON_NEW %in% c('Caltha leptosepala', 'Caltha palustris', 'Trollius europaeus', 'Trollius', 
                       'Thalictrum thalictroides', 'Aquilegia ecalcarata', 
                       'Thalictrum tuberosum') ~ 'Delphinium',
      TAXON_NEW == 'Erigeron' ~ 'Erigeron grandiflorus', 
      TAXON_NEW == 'Scabrethia scabra' ~ 'Wyethia arizonica',
      TAXON_NEW == 'Nemophila breviflora' ~ 'Hydrophyllum',
      TAXON_NEW == 'Epilobium canum' ~ 'Chamaenerion angustifolia',
      TAXON_NEW == 'Paxistima myrsinites' ~ 'Parnassia palustris', #, 
      TAXON_NEW == 'Taraxacum cucullatum' ~ 'Taraxaum oficinale'
    # original was probably right !!! shows the consequence of bad time estimates.#, 
  #   TRUE ~ as.character(TAXON_NEW)
  )) %>% 
  select(TAXON, TAXON_NEW, TAXON_EXPERTS)
