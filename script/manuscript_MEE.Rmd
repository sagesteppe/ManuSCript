...
In order to minimise the number of species for which SDM’s were to be generated, BIEN was queried at a distance of up to 100km from our study area and all plant species records were downloaded. **_In order to emulate the stochasticity of botanical collecting, this dataset was bootstrap re-sampled 250 times, with 90% of samples selected, to create a testing dataset_**. The median of the logistic regression assessing the probability of occurrence of a species record as a function of distance from the study area was used as a threshold distance, under which, to include species as candidates for distribution modelling.
...
```{r Sequence and Morphological Vouchers, out.width = "65%", echo = F,  fig.cap="A caption"}
knitr::include_graphics("../graphics/plots/rmbl_draft_tree.png")
```

*note this figure is draft mode, i reached out to C.H. Cole to get the official APG colors so we are gonna colour edges with that, I have also drawn phylo pics for almost all the labelled order and need to add them in !*
...
Future analyses of the long term dataset... Global studies should aim to corroborate ...
...
- The actual dataset which was used for training and testing all of the models incorporated into SDM's represented only roughly one quarter of the records available for such purposes. We consciously chose to do this in order to showcase the possibility of this approach working in less data rich areas. 
...
**Molecular ... **
- PAFTOL project provide complete genus level reference dataset for the kingdom
...
**Final P**
 < bayesian framework > < harvesting loci > 
...
```{r Morphotype Abundance, echo = F, out.width = '25%'}
