...
abstract: |
  an abstract will be written to fill this space. it will have from 4-6 numbered sections. like 1) A very broad overall identifying organisms is hard and we dont have time. 2) to try and figure this out we used whoa molecular stuff and holy shit amounts of computing 3) Our results showed we were able to do some cool things 4) you should try this out, because it was pretty cool and helped us perform natural history on steroids.
output:
...
54 corbiculae loads had DNA extracted and underwent hyb-seq, a total of `r toString(length(raw_seqs))` corbiculae samples were sequenced, `r toString(sum(raw_seqs))` reads were recovered from sequencing. The number of reads per sequence varied widely (min = `r toString(min(raw_seqs))`, $\bar{x}$ = `r toString(mean(raw_seqs))`, Mdn = `r toString(median(raw_seqs))`, max = `r toString(max(raw_seqs))`), the number of loci recovered per sample ranged from (min = , max = ), and the number of reads from loci also ranged widely () **APPENDIX X**.. After trimming XX sequences remained. `r toString(sum(krak[krak$Classified == 'C',]$n))` reads were matched using Kraken, of the reads classified by Kraken `r toString(sum(brack$reads))` reads were matched using Bracken, of the reads classified by Kraken `r toString(sum(blast$Total_seqs))` reads were matched using BLAST.  

Based upon subjective review of the three classifiers, BLAST was chosen as the classification method which yielded the most probable results, and it's values were used for all analyses. 

```{r, eval = F}
rm(toseq, krak, brack, blast)
```
...
Only 58 of these 388 species (n = 34.56897, Mdn = 31) were able to be compared to ground truth data from long term (1974–2012) datasets.
