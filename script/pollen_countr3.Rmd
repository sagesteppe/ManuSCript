...
pollen_df_new.1 <- pollen_df_new.1 %>%
  ...
  mutate(
    morphotype = str_to_upper(morphotype),
    morphotype = str_replace(morphotype, "CAN\'T_DISTINGUISH", "UNKNOWN"),
    morphotype = str_replace(morphotype, "INDISTINGUISHABLE_CLUMP", "UNKNOWN"),
    morphotype = str_replace(morphotype, "NER", "UNKNOWN"),
    morphotype = str_replace(morphotype, "DEHYDRATED", "UNKNOWN"),
    morphotype = str_replace(morphotype, 'UNKNOWN_[(]5B[)]', 'UNKNOWN'),
    morphotype = str_replace(morphotype, '5B', 'UNKNOWN'),
    morphotype = str_replace(morphotype, 'ASTERACEAE-2', 'ASTERACEAE_2'),
    morphotype = str_replace(morphotype, "SALIX", "D"),
    morphotype = str_replace(morphotype, 'C4', 'C'),
    morphotype = str_replace(morphotype, 'OENOTHERA', 'ONAGRACEAE'),
    morphotype = str_replace(morphotype, 'UNKNOWN_[(]D[)]', 'D'),
    morphotype = str_replace(morphotype, 'UNKNOWN_[(]GROSSULARIACEAE[)]', 'GROSSULARIACEAE'),
    morphotype = str_replace(morphotype, 'OTHER_B', 'B'),
    morphotype = str_replace(morphotype, 'OTHER_C', 'C'),
    morphotype = str_replace(morphotype, "C\\_\\(LINUM\\)", "LINUM"),
    morphotype = str_replace(morphotype, 'UNKNOWN_[(]AMSONIA[?)]', 'UNKNOWN'),
    morphotype = str_replace(morphotype, 'UNKNOWN_[(]RANUNCULUS[)]', 'UNKNOWN'),
    morphotype = str_replace(morphotype, 'UNKNOWN_[(]11B[/]D[)]', 'UNKNOWN'),
    morphotype = str_replace(morphotype, 'UNKNOWN_[(]4_APERTURES[)]', 'UNKNOWN'),
    morphotype = str_replace(morphotype, '^UNKNOWN.*$', 'UNKNOWN'),
    morphotype = str_replace(morphotype, 'UNKNOWN_[(]UNKNOWN[)]', 'UNKNOWN'),
    morphotype = str_replace(morphotype, 'PEDICULARISUNKNOWN', 'PEDICULARIS'),
    morphotype = str_replace_all(morphotype, " ", "_"),
    morphotype = str_remove(morphotype, "\\_\\(11B\\)"),
    morphotype = str_replace_all(morphotype, "UNKNWOWN", "UNKNOWN")) %>% 
  filter(!morphotype %in% c("NO_FJ", "LOW_FJ", "LITTLE_FJ", "LESS_FJ", 'LILTTLE_FJ')) %>%  # we cannot say we counted or scored these cells. 
  filter(!morphotype %in% c(4,8, 11)) %>% 
...
write.csv(pollen_results, '../data/morpho_pollen.csv', row.names = F)
```
...
#[c(1:41,43:44,46:60)]
# 45 does not work
u <- lapply(lapply(to_rare, rowSums), sum)

to_rare <- to_rare[u > 1]
rich_rare <- data.table::rbindlist(lapply(to_rare, raw_vegan), idcol = T)
...
#   Samples 45 iz missing due to only a single morphotype being present per slide. 
ggsave(
